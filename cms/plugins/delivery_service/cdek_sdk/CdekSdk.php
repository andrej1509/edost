...
require (PLUGINS_CODE_DIRECTORY.'delivery_service/cdek_sdk/LSS/Array2XML.php');
require (PLUGINS_CODE_DIRECTORY.'delivery_service/cdek_sdk/LSS/XML2Array.php');
...
class CdekSdk
{
    ...
    protected function call($method, $url, $decodeXml = true, $body = null, array $query = [])
    {
        ...
        if ($pResponse!==false)
        {
            if ($decodeXml) {
                $result = [
                    'result' => XML2Array::createArray($pResponse),
                    'url' => $url
                ];
            } else {
                $result = ['result' => $pResponse, 'url' => $url];
            }
        }
        curl_close($curl);
