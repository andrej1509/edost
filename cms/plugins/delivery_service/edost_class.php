...
define("EDOST_ID", CCMS::$SETS[SETTINGS_SECTION_BITRIX24][SETTINGS_BITRIX24_STORE_ID]); //Идентификатор магазина (вписать вместо: 0)
define("EDOST_PSWD", CCMS::$SETS[SETTINGS_SECTION_BITRIX24][SETTINGS_BITRIX24_ID_PASSWORD]); //Пароль для доступа к серверу расчетов (вписать вместо: password)
//---------- ВНИМАНИЕ!!! ----------

define("EDOST_MAX_CODE", 2828); //максимальный код eDost
...
var $test=0; //включить тестовый вывод =1
	...
	function edost_calc($to_city, $weight, $strah, $length, $width, $height, $zip='', $from_city='') {
		...
		foreach($this->rz as $n){
			...
			$rez['name'.$i] = substr(trim($n['name']),0,120);	//название тарифа (в 2 раза больше из-за UTF)
